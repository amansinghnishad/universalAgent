...
def parse_summary_length_text(summary_length):
    """Parse summary length text and return target word count"""
    import re
    ...
    # Extract numeric value (handles phrases like "Summarize this in 300 words")
    match = re.search(r'(\d+)\s*words?', summary_length)
    if match:
        return int(match.group(1))
    
    # Fallback: extract any number in the string
